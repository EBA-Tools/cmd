...
'	Variables
Dim allowDebugMessages,count(4),curVer,data,dataLoc,dataLocTmp,debugMode,defaultShutdown,desktop,errHandle,eba,enableEndOp,enableLegacyEndOp,endOpFail,exeValue,exeValueExt,fileDir,forVar,forVar1,forVar2,forVar3,forVar4,globTmp,htmlContent,importData,isAdmin,isInstalled,line,lines(5),loadPlugin,logDir,logging,logIn,logInType,maxPlugin,missFiles,nowDate,nowTime,opDetails(7),pcName,pluginCount,prog,programLoc,pWord,regLoc,saveLogin,scriptDir,scriptLoc,shutdownTimer,skipDo,skipExe,startMenu,startup,startupType,status,temp(9),title,uName,user,userType,ver
count(0) = 1
count(4) = 0
...
pluginCount = 0
prog = 0
programLoc = "C:\Program Files\EBA\Cmd"
...
If debugMode = True Then
	...
	errHandle = False
	loadPlugin = False
	db "Debug Mode has been enabled in the scripts ""Modifiable Startup Settings"", which is not recommended. We recommend only using this option for debug usage. Debug mode enables Debug Messages, disables Error Handling, and disables Plugins. If you need help modifying this setting, please contact us."
End If
...
Sub updateCommands
	...
	fileDir = programLoc & "\Commands\database.ebacmd"
	Call update(fileDir,"app.database","overwrite")
	Call update(fileDir,"exe","append")
	Call update(fileDir,"no","append")
	Call update(fileDir,"builtin","append")
